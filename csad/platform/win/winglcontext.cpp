...
namespace csad {
	...
	GlContext::~GlContext()
	{
		...
		bt::MapVoid<void>::iterator it = p_vboMesh.begin();
		bt::MapVoid<void>::iterator end = p_vboMesh.end();
		...
		ObjectsList::iterator itr=Graph::graph().getRenderers()->getObjects().begin();
		ObjectsList::iterator endr=Graph::graph().getRenderers()->getObjects().end();
		while (itr!=endr) {
			Renderer *rend=(Renderer*)itr.value();
			if (rend->getContext()==this) rend->setContext((GlContext*)0);
			itr++;
		}

		((LocalDisplay*)p_parent)->p_context.detachObject(this);
		dfunctions.DestroyContext(p_parent,this);
	...
	bool GlContext::makeCurrent()
	{
		...
		{
#ifdef LOG_CSAD
			Log::console("Make current failed: 0x%x 0x%x", GetLastError(), ffunctions.GetError());
#endif
			return false;
		}
		p_current = this;
