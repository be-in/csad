...
bool initFont(void *parent,Font*font,FT_Face *face)
{
	...
	/*if (FT_Select_Charmap( *face, FT_ENCODING_UNICODE )) {
		Log::console("FT_Select_Charmap: unicode");
		return false;
	}*/
...
unsigned int ASCII(char c)
{
	unsigned char *cd=(unsigned char *)&c;
	if (*cd>=0xC0) return *cd-0xC0+0x410;
	if (*cd==0xA8) return 0x401;
	if (*cd==0xB8) return 0x451;
	return *cd;
}

float Font::width(char simvol)
{
	FT_Face face;

	if (initFont(p_parent->p_pf,this,&face)) {
		FT_UInt glyph_index = FT_Get_Char_Index( face, ASCII(simvol) );
...
void Font::rasterText(void *raster,int x,int y,unsigned int color,int mode,char*text)
{
	...
	if (initFont(p_parent->p_pf,this,&face)) {
		...
		while (*text) {
			...
			FT_UInt glyph_index = FT_Get_Char_Index( face, ASCII(*text) );
			FT_Load_Glyph( face,  glyph_index, FT_LOAD_DEFAULT );
			FT_Glyph glyph;
