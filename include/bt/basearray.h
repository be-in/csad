...
public:
	...
	public:
		...
		_FORCEINLINE iterator(const iterator &it) {mIterator=it.mIterator;d=it.d;}
		_FORCEINLINE iterator(iterator &it) {mIterator=it.mIterator;d=it.d;}
		~iterator();
...
public:
	...
	char length() {if (!mData) return 0; return mData[0];}
...
public:
	...
	unsigned int length() {if (!mData) return 0; return *(unsigned int*)mData;}
